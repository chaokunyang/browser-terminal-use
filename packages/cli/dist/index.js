...
async function runExec(options, args) {
...
async function runCancel(options, args) {
    ...
    ws.send(stringifyMessage(payload));
    const result = await waitForMessage(ws, (message) => (message.type === "exec_cancelled" && message.requestId === requestId) ||
...
async function connectCli(options) {
    ...
    await new Promise((resolve, reject) => {
        ...
        const onMessage = (buffer, isBinary) => {
            if (isBinary) {
                return;
            }
            const message = safeParseMessage(buffer.toString("utf8"));
...
async function waitForMessage(ws, predicate, timeoutMs = 6000) {
