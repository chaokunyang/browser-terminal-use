...
import {
  ...
  type ClientRole,
  type CancelMessage,
  type ExecCancelMessage,
  type ExecErrorMessage,
  type ExecMessage,
  type ExecRequestMessage,
  type ExecResultMessage,
  type ExecStartedMessage,
  type ExecOutputMessage,
  type ExecCancelledMessage,
  type TerminalStatusMessage,
  type ErrorMessage,
  type HealthResultMessage,
  type HelloMessage,
...
export class BridgeServer {
  ...
  private readonly cliSockets = new Set<WebSocket>();
  ...
  async start(): Promise<void> {
    ...
    this.log(
      "info",
      `bridge listening on ws://${this.config.host}:${this.config.port} (cli=/cli, extension=/extension)`
    );
  ...
  private handleUpgrade(
    ...
    const roleHint: ClientRole = pathname === "/cli" ? "cli" : "extension";
  ...
  private onSocketConnection(ws: WebSocket, roleHint: ClientRole): void {
    ...
    ws.on("message", (buffer: RawData, isBinary: boolean) => {
      if (isBinary) {
        this.sendError(ws, {
          type: "error",
      ...
      if (!state.authed) {
        ...
        const ok = this.authenticate(ws, message as HelloMessage, state);
        if (!ok) {
          return;
        }
      ...
      this.onAuthedMessage(ws, state, message as BridgeInboundMessage);
    });
  ...
  private authenticate(ws: WebSocket, hello: HelloMessage, state: SocketState): boolean {
  ...
  private onAuthedMessage(ws: WebSocket, state: SocketState, message: BridgeInboundMessage): void {
  ...
  private handleCliMessage(ws: WebSocket, message: BridgeInboundMessage): void {
    if (message.type === "health") {
      const health: HealthResultMessage = {
        type: "health_result",
        ...this.healthPayload()
      };
      this.send(ws, health);
    ...
    if (message.type === "exec") {
      this.enqueueExec(ws, message as ExecMessage);
      return;
    }

    if (message.type === "cancel") {
      this.cancelRequest(ws, message as CancelMessage);
      return;
    }
  ...
  private enqueueExec(ws: WebSocket, message: ExecMessage): void {
    ...
    if (!this.extensionSocket || this.extensionSocket.readyState !== WebSocket.OPEN) {
      const payload: ExecErrorMessage = {
        type: "exec_error",
        requestId,
  ...
  private cancelRequest(ws: WebSocket, message: CancelMessage): void {
    ...
    if (request.status === "running") {
      if (this.extensionSocket) {
        const payload: ExecCancelMessage = {
          type: "exec_cancel",
          requestId: request.requestId,
  ...
  private handleExtensionMessage(message: BridgeInboundMessage): void {
    if (message.type === "terminal_status") {
      const status = message as TerminalStatusMessage;
      this.log(
        "debug",
        `terminal status: count=${status.terminalCount}, active=${status.activeTabId}, bound=${status.boundTabId}`
      );
      return;
    }

    if (
      message.type === "exec_started" ||
      message.type === "exec_output" ||
      message.type === "exec_result" ||
      message.type === "exec_error" ||
      message.type === "exec_cancelled"
    ) {
      this.forwardExecutionEvent(message as ExecStartedMessage | ExecOutputMessage | ExecResultMessage | ExecErrorMessage | ExecCancelledMessage);
      return;
    }
  ...
  private forwardExecutionEvent(
    message: ExecStartedMessage | ExecOutputMessage | ExecResultMessage | ExecErrorMessage | ExecCancelledMessage
  ): void {
    const request = this.requests.get(message.requestId);
    if (!request) {
  ...
  private dispatchQueue(): void {
    ...
    while (this.queue.length > 0) {
      ...
      const payload: ExecRequestMessage = {
        type: "exec_request",
        requestId,
  ...
  private onSocketClosed(ws: WebSocket, state: SocketState): void {
    if (state.role === "cli") {
  ...
  private cancelRequestsOwnedByCli(ws: WebSocket): void {
  ...
  private healthPayload(): Omit<HealthResultMessage, "type"> {
    return {
  ...
  private send(ws: WebSocket, payload: BridgeOutboundMessage | BridgeInboundMessage): void {
  ...
  private sendError(ws: WebSocket, message: ErrorMessage): void {
...
function normalizeTimeout(input: number | undefined, fallback: number, max: number): number {
...
function removeFromQueue(queue: string[], requestId: string): void {
